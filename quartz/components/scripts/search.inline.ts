...
async function setupSearch(
  ...
  const formatForDisplay = (term: string, id: number) => {
    ...
    return {
      ...
      contentType: data[slug].contentType ?? "页面",
    };
  };
  ...
  const resultToHTML = ({ slug, title, content, tags, contentType }: Item) => {
    const htmlTags =
    ...
    itemTile.innerHTML = `
      <p class="search-content-type">${contentType ?? "页面"}</p>
  ...
  for (const [slug, fileData] of Object.entries<ContentDetails>(data)) {
    promises.push(
      index.addAsync(id++, {
        ...
        tags: fileData.tags,
        contentType: fileData.contentType ?? "页面",
      }),
    );
  }
